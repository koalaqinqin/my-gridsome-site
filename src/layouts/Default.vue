<template>
    <div>
        <section class="page-header" :style="'background-image: linear-gradient(120deg, '+config.backgroundColorLeft+', '+config.backgroundColorRight+');color: '+config.fontColor+';'">
                ...
                <el-row>
                    <el-col :span="10">
                        <el-menu @select="selectTopbar" :default-active="topbar.active" mode="horizontal" menu-trigger="click">
                            ...
                            <el-submenu index="#webSites" v-if="config.webSites.length>0">
                                ...
                                <el-menu-item :index="'#webSites-'+index" v-for="(item,index) in config.webSites" :key="'#webSites'+index">{{item.name}}</el-menu-item>
                            </el-submenu>
                        </el-menu>
                    </el-col>
                    ...
                    <el-col :span="4" style="text-align: right;">
                        ...
                        <div style="color:#606266;">
                            <i class="el-icon-location"></i>&nbsp;{{user.location?user.location:'未填写地址'}}
                            <br>
                        </div>
                    </el-col>
        ...
        methods: {
            selectTopbar(index) {
                ...
                switch (index) {
                    ...
                    case "#blog":
                        if (this.user.blog) {
                            window.open((this.user.blog.match(/https?:\/\//i)?'':'https://') + this.user.blog)
                        } else {
                            this.$message({
                                message: '博主没有其他博客',
                                type: 'info'
